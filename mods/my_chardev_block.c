/*  my_chardev_block.c - Simple character device module
 *  
 *  Demonstrates module creation of character device for user
 *  interaction.  Adds blocking I/O, multiple open's will block rather
 *  than fail.
 *
...
int my_init(void)
{
  /* initialize the waitqueue we will use to block when the device is
     in use. */
...
static int device_open(struct inode *inode, struct file *file)
{
  ...
  if (down_interruptible (&sem))
    return -ERESTARTSYS;	/* If we return from
				   down_interruptible with a non-zero
				   value the program must have
				   received a signal. Let the kernel
				   restart this operation in that
				   case. */
  

  /* We are only allowing one process to hold the device file open
     at a time. */
  while (Device_Open){		/* now we loop here on flag */
      up(&sem);			/* Release your semaphores if we are
				   going to block */
      if (wait_event_interruptible(my_waitqueue, (Device_Open == 0)))
	return -ERESTARTSYS; /* interrupted by signal: tell the fs
				layer to handle it */

      /* Reaquire the lock so we can safely test and change Device
	 Open on the next time through the while loop */
      if (down_interruptible (&sem))
	return -ERESTARTSYS;
    }
...
static int device_release(struct inode *inode, struct file *file)
{
  ...
  /* wake up one sleeper waiting for Device_Open flag */
